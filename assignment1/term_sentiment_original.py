...
def parseTweetData():
    for key in term_data:
        ...
        for i in range(len(twit)):
            # crude filter for @user and retweets
            if (twit[i].startswith('@') or twit[i] == 'RT'):
                continue
            # more crude filtration
            # tre = re.findall(r"[a-zA-Z]+[']?[a-zA-Z]+", twit[i])
            # if tre:
            #     term = tre[0].lower()
            #     if term not in sentiment_data:
